...
/*DROP TABLE IF EXISTS links_imp;
CREATE TABLE links_imp AS
SELECT
        link_id,
        --from_id,
        --to_id,
        ...
        permlanes,
        --modes,
        (SELECT urbrurfunc(wkb_geometry => public.to2018_swe99tm.wkb_geometry)) AS area,
...
FROM scenario_1_2_nvdb.links 
LEFT JOIN public.to2018_swe99tm
        ON scenario_1_2_nvdb.links.geom && public.to2018_swe99tm.wkb_geometry
WHERE modes = 'car';
...
SELECT
        ...
        'TBD' AS road_type, -- Motorway-Nat., Motorway-City, Semi-Motorway, Primary-nat. non-motorway, Primary-city non-motorway, Distributor/Secondary, Distributor/Secondary(sinuous), Local/Collector, Local/Collector(sinuous), Access-residential
        links.speed_limit,
        (SELECT capfunc(capacity => links.capacity, flow => car_flow_by_hour.flow)) AS level_of_service
FROM scenario_1_2_nvdb.link_movement AS link_movement FULL JOIN public.links_imp AS links
